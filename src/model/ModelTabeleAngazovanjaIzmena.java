 ...
 * @author Mafija
 */
public class ModelTabeleAngazovanjaIzmena extends AbstractTableModel{

    ArrayList<Angazovanje> lista;
    SimpleDateFormat sdf = new SimpleDateFormat("dd.MM.yyyy");
    
    public ModelTabeleAngazovanjaIzmena(){
        lista = new ArrayList<>();
    }

    public ModelTabeleAngazovanjaIzmena(ArrayList<Angazovanje> lista) {
        this.lista = lista;
    ...
    public void obrisiAngazovanje(int red){
        Angazovanje a = lista.get(red);
        if (a.getStatus().equals("NOVI")) {
           lista.remove(red);
        }else{
            lista.get(red).setStatus("BRISANJE");
        }
        fireTableDataChanged();
    }
    ...
    public void izmeniAngazovanje(Angazovanje a) {
        for (Angazovanje angazovanje : lista) {
            if (angazovanje.getId() == a.getId()) {
                angazovanje.setDatum(a.getDatum());
                angazovanje.setStatus("IZMENA");
            }
        }
        fireTableDataChanged();
    }
